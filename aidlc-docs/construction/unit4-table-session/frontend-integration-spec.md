...
- [ ] **프론트엔드 테스트 하네스 추가** — 병렬 개발 단계에서는 의도적으로 보류(공유 골격 부재로 독립 빌드 불가). 공유 admin 골격(Vitest/vue-test-utils) 확립 후, 순수 로직(`http.ts` URL·에러 파싱 등)부터 단위 테스트 추가. 백엔드는 `pytest`로 검증 완료(API 12 + PBT 3).
